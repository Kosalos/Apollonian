...
class SliderView: UIView {
    ...
    func initializeInt32(_ v: inout Int32, _ sType:SliderType, _ min:Float, _ max:Float,  _ delta:Float, _ iname:String) {
        ...
        deltaValue = delta
        name = iname
        sSize = Float(bounds.width)
        vCenter = sSize / 2
        setNeedsDisplay()
    }

    func initializeFloat(_ vx:UnsafeMutableRawPointer, _ sType:SliderType, _ min:Float, _ max:Float,  _ delta:Float, _ iname:String) {
        valuePointer = vx
        valuetype = .float
        slidertype = sType
        mRange.x = min
    ...
    override func draw(_ rect: CGRect) {
        ...
        func coloredValue(_ v:Float) { drawText(vx,textY,valueColor(v),16, formatted(v)) }
        
        if valuePointer != nil {
            drawText(10,textY,.white,16,name)
            
            switch valuetype {
            case .int32 :
                let v:Int32 = valuePointer.load(as: Int32.self)
                drawText(percentX(0.75),textY,.gray,16, v.description)
            case .float :
                let v:Float = valuePointer.load(as: Float.self)
                
                if v > 100 {
                    drawText(vx,textY,.gray,16, formatted3(v))
                }
                else {
                    coloredValue(v)
                }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for t in touches {
            ...
            if slidertype == .direct {
                let value = fClamp(mRange.x + (mRange.y - mRange.x) * Float(pt.x) / sSize, mRange)
                ...
                delta = (Float(pt.x) - vCenter) / sSize / 10
            }
