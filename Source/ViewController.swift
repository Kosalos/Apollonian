...
class ViewController: UIViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        dCameraXY.initializeFloat1(&cameraX, cameraMin, cameraMax, cameraDelta, "Cam XY")  // cameraX
        dCameraXY.initializeFloat2(&cameraY)
        sCameraZ.initializeFloat(&cameraZ, .delta, cameraMin, cameraMax,cameraDelta, "Cam Z")
        dFocusXY.initializeFloat1(&focusX, focusMin,focusMax,cameraDelta, "Foc XY")
        dFocusXY.initializeFloat2(&focusY)
        sFocusZ.initializeFloat(&focusZ, .delta, focusMin, focusMax, cameraDelta, "Foc Z")
    ...
    @objc func layoutViews() {
        ...
        let cys:CGFloat = 100
        let bys:CGFloat = 30    // slider height
        let gap:CGFloat = 5
        ...
        x = x2
        y += 20
        circleButton.frame  = frame(cxs,bys,0,bys+gap+10)
        resetButton.frame  = frame(cxs,bys,0,bys+gap)
    }
    
    func reset() {
        control.camera = vector_float3(2.8,-1.66,1.194)
        control.focus = vector_float3(-1.02,-0.76,-0.734)
        control.zoom = 0.6141
